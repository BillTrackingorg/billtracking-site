...
The pages here additionally run the **compiled core,
...
files: `assets/js/bt-web.js`, the compiled core; and `assets/js/frame-guard.js`,
which blanks the page if it has been loaded inside a frame.

*(Corrected 2026-09-08, three times over. The first version said "the feed, vote and account pages …
...
And the third correction is the point of the rule: the replacement said "on every
page", which was ALSO a page-set claim and ALSO false — four pages generated by
the permalinks workflow (`/corrections/`, `/p/`, `/b/us/`, `/b/eu/`) carried no
script at all. The count was swapped for a different count in the same edit. The
quantifiers are gone; the files are named and nothing is claimed about where they
load, which is the sentence this file's own rule allows.)*
